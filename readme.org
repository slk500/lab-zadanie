...
- *Wymagania techniczne:*
  - Obsługa błędów w przypadku:
    - niekompletnego 
    - lub wadliwego pliku CSV.
  - Logowanie poprawnie zaimportowanych rekordów oraz błędów do pliku.

*** 2. Stworzenie API

- *Endpointy:*
  - =POST /api/login= -- logowanie użytkownika na podstawie
    loginu (połączenie imienia i nazwiska pacjenta,
    np. =PiotrKowalski=) i hasła (data urodzenia pacjenta,
    np. =1983-04-12=). Zwraca token JWT.
  - =GET /api/results= -- zwraca dane zalogowanego pacjenta
    oraz wyniki jego badań na podstawie tokenu JWT.
...
*** 6. Docker
- Opracuj plik =docker-compose.yml=, który umożliwi lokalne
  uruchomienie aplikacji z backendem, frontendem i bazą
  danych.

** Czas Realizacji
Zadanie należy wykonać w ciągu *7 dni* od momentu jego
otrzymania.
** Wynik Końcowy
Kandydat powinien dostarczyć repozytorium GIT (np. link do
GitHub/GitLab/Bitbucket), które zawiera: - Kod źródłowy
backendu i frontendu. - Pliki konfiguracyjne Docker, CI/CD i
migracji. - Plik README.md z instrukcjami uruchomienia
projektu i pipeline'a CI/CD.
